...
-- 2024-08-05,MMcDonald,Created File
--**************************************************************************--
Begin Try
	Use Master;
	If Exists(Select Name From SysDatabases Where Name = 'Assignment07DB_MMcDonald')
	 Begin 
	  Alter Database [Assignment07DB_MMcDonald] set Single_user With Rollback Immediate;
	  Drop Database Assignment07DB_MMcDonald;
	 End
	Create Database Assignment07DB_MMcDonald;
End Try
Begin Catch
	Print Error_Number();
End Catch
go
Use Assignment07DB_MMcDonald;
...
SELECT
	[Product Name] = ProductName,
	[Inventory Date] = DATENAME(mm, i.InventoryDate)+', '+DATENAME(YYYY, i.InventoryDate),
...
SELECT TOP 1000000000
	...
	[Inventory Date] = DATENAME(mm, i.InventoryDate)+', '+DATENAME(YYYY, i.InventoryDate),
	[Inventory Count] = i.Count,
	[Previous Month Count] = ISNULL(LAG(Count) OVER(PARTITION BY p.ProductName ORDER BY i.InventoryDate), 0)
FROM vProducts as p 
JOIN vInventories as i 
ON p.ProductID = i.ProductID
ORDER BY ProductName, InventoryDate
...
SELECT*FROM fProductInventoriesWithPreviousMonthCountsWithKPIs(-1);
GO 
--results already ordered by product and date in code used for view vProductInventoriesWithPreviouMonthCounts
